const artifact = require('@rsksmart/dnssec-oracle/build/contracts/DNSSECImpl.json');
...
class Oracle {
  ...
  async knownProof(proof) {
    ...
    let toProve;
    if (proof.rrdata) {
      toProve = this.web3.utils
        .sha3('0x' + proof.rrdata.toString('hex'), { encoding: 'hex' })
        .slice(0, 42);
    }
   ...
   * @param {string} name - eg: _rns.matoken.xyz
   * @param {string} proof
  ...
  async getProven(result) {
    ...
    for (i = result.proofs.length - 1; i >= 0; i--) {
      let minInserted =
        Date.now() / 1000 -
        Math.min(...result.results[i].rrs.map(rr => rr.ttl));
      let proof = result.proofs[i];
      let proven = await this.knownProof(proof);
      if (proven.matched && proven.inserted > minInserted) break;
    }
    return i + 1;
  }
}
